...
class DatabaseControllerTests {
    ...
    void testPostCat() throws Exception {
        ...
        // Object to String
        // String contentString = new ObjectMapper().writeValueAsString(mockedCreatedCat);

        // Mock Service postCat
        // catDatabaseService.createCatToStaticMemory(idx, cat);
        Mockito.when(
                catDatabaseService.createCatToStaticMemory(1, mockedCreatedCat))
                .thenReturn(mockedCreatedCat);
    ...
    void testDeleteCat() throws Exception {
        ...
        Mockito.when(
                catDatabaseService.deleteCatFromStaticMemory(1))
                .thenReturn(true);
    ...
    void testputCat() throws Exception {
        ...
        Mockito.when(
                catDatabaseService.putCatToStaticMemory(1, mockedCreatedCat))
                .thenReturn(mockedCreatedCat);
    ...
    void testpatchCat1() throws Exception {
        ...
        Mockito.when(
                catDatabaseService.patchCatToStaticMemory(1, "Thomas"))
                .thenReturn(mockedPatchCat);

        // suimulate patch commend in postmen
        mockMvc.perform(patch("/api/v1/cat/{idx}/name/{name}", "1", "Thomas"))
                .andExpect(status().isOk()) // show server status: 200 = OK, 404 = Error: path not found, 500 = IP not found or Port not found
                .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
    ...
    void testpatchCat2() throws Exception {
        ...
        // Cat patchCatToStaticMemory(int index, String name)
        Mockito.when(
                catDatabaseService.patchCatToStaticMemory(1, 10))
                .thenReturn(mockedPatchCat);
